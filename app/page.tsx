...
const museumEnhancements={
  de:{guided:'Geführte Experience · ca. 5 Minuten',free:'Frei erkunden',guidedLabel:'Geführter Weg',next:'Nächster Moment',exit:'Geführten Weg verlassen',salonStations:[['Stofftisch','Stofflagen öffnen'],['Schneiderpuppe','Schnitt entstehen lassen'],['Geschäftstisch','Dokumente entdecken'],['Spiegel','Flöges Perspektive']],salonInsights:[['Material, Fall und Herkunft','Öffne die Stofflagen: Die Proben unterscheiden sich in Gewicht, Oberfläche und Herkunft. Diese Eigenschaften beeinflussen, wie ein Kleid fällt und auf Bewegung reagiert.','Material ist eine gestalterische und wirtschaftliche Entscheidung.'],['Vom Schnitt zum Bewegungsraum','Die Schnittlinien erscheinen nacheinander auf der Schneiderpuppe. Werden die Teile weiter gesetzt, gewinnt die Silhouette an Volumen und der Körper an möglichem Bewegungsraum.','Der Schnitt organisiert Form und Bewegung.'],['Ein Salon als Unternehmen','Rechnung, Kundinnenkarte und Entwurf werden sichtbar. Sie stehen für Beratung, Auftragsplanung, Mitarbeiterinnen und wirtschaftliche Verantwortung im Salon.','Hinter jedem Entwurf stand unternehmerische Arbeit.'],['Emilie Flöge im eigenen Salon','Der Spiegel richtet den Blick auf Flöge selbst: 1904 gründete sie mit ihren Schwestern Pauline und Helene den Modesalon „Schwestern Flöge“ und leitete ihn als Gestalterin und Unternehmerin.','Ihre Arbeit verband Entwurf, Kundinnen und selbstständiges Wirtschaften.']],nuance:'Flöges weit geschnittene Entwürfe standen für eine andere Vorstellung von Silhouette und Bewegungsraum. Wie frei Kleidung im Alltag erlebt wurde, hing zugleich von Material, gesellschaftlicher Situation und der jeweiligen Trägerin ab.',postcard:'Mein Entwurf aus dem digitalen Modeatelier',name:'Name (optional)',namePlaceholder:'Dein Name',movement:'Gewählte Bewegung',space:'Erreichter Bewegungsraum',principle:'Inspiriert von Flöges Verbindung aus Schnitt, Stoff und möglichem Bewegungsraum.',date:'Entstanden am'},
  en:{guided:'Guided experience · about 5 minutes',free:'Explore freely',guidedLabel:'Guided path',next:'Next moment',exit:'Leave guided path',salonStations:[['Fabric table','Open the fabric layers'],['Dress form','Reveal the cut'],['Business desk','Discover the documents'],['Mirror','Flöge’s perspective']],salonInsights:[['Material, drape, and origin','Open the fabric layers: the samples differ in weight, surface, and origin. These qualities influence how a dress falls and responds to movement.','Material is both a design and business decision.'],['From cut to range of movement','The cutting lines appear one by one on the dress form. Placing the pieces more widely gives the silhouette volume and the body potential room to move.','The cut organises form and movement.'],['A salon as a business','An invoice, client card, and design become visible. They represent consultation, commissions, employees, and commercial responsibility in the salon.','Every design was supported by entrepreneurial work.'],['Emilie Flöge in her own salon','The mirror turns the focus to Flöge herself: in 1904, she founded the Schwestern Flöge fashion salon with her sisters Pauline and Helene and ran it as a designer and entrepreneur.','Her work connected design, clients, and independent business.']],nuance:'Flöge’s loose-cut designs represented a different idea of silhouette and range of movement. How free clothing felt in everyday life also depended on its material, the social situation, and the individual wearer.',postcard:'My design from the digital fashion atelier',name:'Name (optional)',namePlaceholder:'Your name',movement:'Selected movement',space:'Achieved range of movement',principle:'Inspired by Flöge’s connection between cut, fabric, and potential range of movement.',date:'Created on'},
  fr:{guided:'Expérience guidée · environ 5 minutes',free:'Explorer librement',guidedLabel:'Parcours guidé',next:'Étape suivante',exit:'Quitter le parcours guidé',salonStations:[['Table des étoffes','Ouvrir les couches'],['Mannequin','Révéler la coupe'],['Bureau','Découvrir les documents'],['Miroir','Le regard de Flöge']],salonInsights:[['Matière, tombé et origine','Ouvrez les couches : les échantillons diffèrent par leur poids, leur surface et leur origine. Ces qualités influencent le tombé de la robe et sa réaction au mouvement.','La matière est une décision créative et économique.'],['De la coupe à l’espace du mouvement','Les lignes de coupe apparaissent successivement sur le mannequin. En espaçant les pièces, la silhouette gagne en volume et le corps en espace potentiel.','La coupe organise la forme et le mouvement.'],['Un salon comme entreprise','Une facture, une fiche cliente et un dessin apparaissent. Ils évoquent le conseil, les commandes, les employées et la responsabilité économique du salon.','Chaque création reposait sur un travail entrepreneurial.'],['Emilie Flöge dans son salon','Le miroir ramène le regard vers Flöge : en 1904, elle fonde avec ses sœurs Pauline et Helene le salon Schwestern Flöge, qu’elle dirige comme créatrice et entrepreneuse.','Son travail reliait création, clientèle et entreprise indépendante.']],nuance:'Les créations amples de Flöge proposaient une autre conception de la silhouette et de l’espace du mouvement. La liberté ressentie au quotidien dépendait aussi de la matière, de la situation sociale et de chaque personne qui portait le vêtement.',postcard:'Ma création de l’atelier de mode numérique',name:'Nom (facultatif)',namePlaceholder:'Votre nom',movement:'Mouvement choisi',space:'Liberté de mouvement obtenue',principle:'Inspiré du lien établi par Flöge entre coupe, étoffe et espace potentiel du mouvement.',date:'Créé le'}
} as const;
const salonCountCopy={de:{explore:'Vier Spuren im Raum',done:'Du hast alle vier Bereiche entdeckt.'},en:{explore:'Four traces in the room',done:'You have discovered all four areas.'},fr:{explore:'Quatre traces dans l’espace',done:'Vous avez découvert les quatre espaces.'}} as const;
...
export default function Home() {
  ...
  const [salonDiscoveries, setSalonDiscoveries] = useState<boolean[]>([false,false,false,false]);
  ...
  const m = museumEnhancements[lang];
  ...
  return (
    <main className={`experience-shell chapter-${chapter} ${entered?'has-entered':''} ${released ? 'is-released' : ''} ${reducedMotion?'reduce-motion':''}`}>
      <section className={`museum-portal ${entered?'is-entered':''}`} aria-hidden={entered}>
        ...
        <div className="portal-copy"><p>{c.portalEye}</p><h1>{c.portalTitle}</h1><p className="portal-intro">{s.portalIntro}</p><div className="portal-paths"><button type="button" onClick={()=>{setGuidedMode(true);setEntered(true);salonEntrance()}}><span>{m.guided}</span><i aria-hidden="true">→</i></button><button className="portal-free" type="button" onClick={()=>{setGuidedMode(false);setEntered(true);salonEntrance()}}><span>{m.free}</span></button></div></div>
      ...
      {guidedMode&&entered&&<nav className="guided-rail" aria-label={m.guidedLabel}><span><small>{m.guidedLabel}</small><strong>0{chapter+1} / 07</strong></span><i aria-hidden="true"><b style={{width:`${((chapter+1)/7)*100}%`}}/></i>{chapter<6?<button type="button" onClick={()=>moveToChapter(chapter+1)}>{m.next}<span aria-hidden="true">↓</span></button>:<button type="button" onClick={()=>setGuidedMode(false)}>{m.exit}</button>}<button className="guided-exit" type="button" onClick={()=>setGuidedMode(false)} aria-label={m.exit}>×</button></nav>}
      ...
      <section className="context-scene scene" id="einordnen" aria-labelledby="context-title">
        ...
        <div className="context-copy">
          ...
          <p className="historical-nuance">{m.nuance}</p>
      ...
      <section className={`salon-scene scene ${salonEntered?'is-inside':'is-outside'} ${salonDiscoveries.every(Boolean)?'is-complete':''} ${activeSalonStation!==null?`has-active-station active-station-${activeSalonStation}`:''}`} id="salon" aria-labelledby="salon-title">
        ...
        <div className="salon-room" onPointerMove={event=>{const rect=event.currentTarget.getBoundingClientRect();event.currentTarget.style.setProperty('--room-x',`${((event.clientX-rect.left)/rect.width-.5)*2}`);event.currentTarget.style.setProperty('--room-y',`${((event.clientY-rect.top)/rect.height-.5)*2}`)}}>
          ...
          <p className="salon-progress">{salonCountCopy[lang].explore} · {salonDiscoveries.filter(Boolean).length}/4</p>
          <div className="salon-stations">
            {m.salonStations.map((station,index)=><button key={station[0]} type="button" className={`salon-station station-${index} ${salonDiscoveries[index]?'is-found':''} ${activeSalonStation===index?'is-active':''}`} aria-pressed={activeSalonStation===index} onClick={()=>{setActiveSalonStation(index);setSalonDiscoveries(items=>items.map((item,i)=>i===index?true:item));if(index===0)noiseGesture(.8,.04,1700);if(index===1){soundPulse(190,.5);openAir()}if(index===2)businessRhythm();if(index===3){soundPulse(392,.7);noiseGesture(.5,.012,2100)}}}><span className="station-object" aria-hidden="true">{index===0?<span className="fabric-discovery"><i/><i/><i/><b/></span>:index===1?<span className="cut-discovery"><img src="./visuals/schneiderpuppe-atelier.png" alt=""/><i/><i/><i/></span>:index===2?<span className="business-discovery"><i/><i/><b/><em/></span>:<span className="mirror-discovery"><i/><img src="./visuals/emilie-portrait.png" alt=""/><b/></span>}</span><span className="station-label"><small>0{index+1}</small><strong>{station[0]}</strong><em>{station[1]}</em></span></button>)}
          </div>
          {activeSalonStation!==null&&<aside className={`salon-insight insight-${activeSalonStation}`} aria-live="polite"><button type="button" className="salon-insight-close" aria-label={lang==='de'?'Detail schließen':lang==='fr'?'Fermer le détail':'Close detail'} onClick={()=>setActiveSalonStation(null)}>×</button><small>{c.salonInsightEye} · 0{activeSalonStation+1}</small><div className="insight-artifact" aria-hidden="true">{activeSalonStation===0?<><i/><i/><i/></>:activeSalonStation===1?<><i/><i/><i/><b/></>:activeSalonStation===2?<><i/><i/><b/></>:<><i/><img src="./visuals/emilie-portrait.png" alt=""/></>}</div><h3>{m.salonInsights[activeSalonStation][0]}</h3><p>{m.salonInsights[activeSalonStation][1]}</p><strong>{m.salonInsights[activeSalonStation][2]}</strong></aside>}
          {salonDiscoveries.every(Boolean)&&<div className="salon-complete" aria-live="polite"><strong>{salonCountCopy[lang].done}</strong><button type="button" onClick={()=>document.querySelector('#unternehmen')?.scrollIntoView({behavior:'smooth'})}>{c.salonContinue}<span>→</span></button></div>}
        </div>
      </section>
      ...
      <section className="atelier-scene scene" id="anwenden" aria-labelledby="atelier-title">
        ...
        <aside className="atelier-right">
          ...
          <StudyIdentity intention={intention} setIntention={setIntention} rhythm={patternRhythm} setRhythm={setPatternRhythm} accent={patternAccent} setAccent={setPatternAccent} shape={shape} action={action} actionLabel={c.actions[['gehen','arbeiten','tanzen'].indexOf(action)]} mobility={mobility} labels={x} postcard={m} visitorName={visitorName} setVisitorName={setVisitorName} onDownload={downloadStudy} onShare={shareStudy} shareStatus={shareStatus}/>
        </aside>
      </section>
