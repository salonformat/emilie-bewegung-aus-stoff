...
export default function Home() {
  ...
  return (
    <main className={`experience-shell chapter-${chapter} ${entered?'has-entered':''} ${released ? 'is-released' : ''} ${reducedMotion?'reduce-motion':''}`}>
      ...
      <section className="atelier-output-section" id="entwurf" aria-label={x.resultEye}>
        <div className="personal-study"><p className="eyebrow">{x.resultEye}</p><h3>{x.resultTitle}</h3><StudyPreview shape={shape} intention={intention} rhythm={patternRhythm} accent={patternAccent} action={action}/><p><strong>{c.actions[['gehen','arbeiten','tanzen'].indexOf(action)]}</strong> · {mobility}% {c.mobility}</p><p>{intention.length?<><strong>{motifLabel}</strong> · {x.rhythms[patternRhythm]} · {x.accents[patternAccent]}</>:blankPatternCopy[lang]}</p><p>{x.resultThought}</p><div><button type="button" onClick={downloadStudy} disabled={!intention.length}>{x.download}</button><button type="button" onClick={shareStudy} disabled={!intention.length}>{x.share}</button></div><small aria-live="polite">{shareStatus}</small></div>
      </section>
...
function StudyIdentity({intention,setIntention,rhythm,setRhythm,accent,setAccent,shape,action,labels}:{intention:number[];setIntention:(v:number[])=>void;rhythm:number;setRhythm:(v:number)=>void;accent:number;setAccent:(v:number)=>void;shape:{width:number;weight:number;layers:number};action:'gehen'|'arbeiten'|'tanzen';labels:typeof extraCopy.de|typeof extraCopy.en|typeof extraCopy.fr}){const locale=labels.intentions[0]==='Rechteck'?'de':labels.intentions[0]==='Square'?'en':'fr',motifLabel=intention.map(item=>labels.intentions[item]).join(' + '),toggleMotif=(motif:number)=>setIntention(intention.includes(motif)?intention.filter(item=>item!==motif):[...intention,motif].sort());return <fieldset className="signature-maker identity-maker"><legend>{labels.signatureEye}</legend><h3>{labels.signatureTitle}</h3><p>{labels.signatureBody}</p><div className="pattern-workbench"><StudyPreview shape={shape} intention={intention} rhythm={rhythm} accent={accent} action={action}/><div className="pattern-choices"><span>1 · {labels.motif}</span><div className="intention-picker">{labels.intentions.map((item,i)=><button key={item} type="button" aria-pressed={intention.includes(i)} onClick={()=>toggleMotif(i)}><i className={`motif-icon motif-icon-${i}`} aria-hidden="true"/>{item}</button>)}</div><span>2 · {labels.statement}</span><div className="intention-picker">{labels.rhythms.map((item,i)=><button key={item} type="button" aria-pressed={rhythm===i} onClick={()=>setRhythm(i)} disabled={!intention.length}>{item}</button>)}</div><span>3 · {labels.accent}</span><div className="intention-picker accent-picker">{labels.accents.map((item,i)=><button key={item} type="button" aria-pressed={accent===i} onClick={()=>setAccent(i)} disabled={!intention.length}><i className={`accent-dot accent-dot-${i}`} aria-hidden="true"/>{item}</button>)}</div></div></div><p className="pattern-connection">{intention.length?<strong>{motifLabel} · {labels.rhythms[rhythm]} · {labels.accents[accent]}</strong>:blankPatternCopy[locale]}</p><div className="pattern-actions"><button className="pattern-apply" type="button" disabled={!intention.length} onClick={()=>document.querySelector('#entwurf')?.scrollIntoView({behavior:'smooth'})}>{labels.signatureGenerate} →</button><button className="identity-reset" type="button" onClick={()=>{setIntention([]);setRhythm(0);setAccent(0)}}>{labels.signatureClear}</button></div></fieldset>}
