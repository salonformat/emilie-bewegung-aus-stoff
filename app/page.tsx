    ...
    <main className={`experience-shell chapter-${chapter} ${entered?'has-entered':''} ${released ? 'is-released' : ''}`}>
      ...
      <section className={`salon-scene scene ${salonEntered?'is-inside':'is-outside'} ${salonDiscoveries.every(Boolean)?'is-complete':''} ${activeSalonStation!==null?`has-active-station active-station-${activeSalonStation}`:''}`} id="salon" aria-labelledby="salon-title">
        ...
        <div className="salon-room" onPointerMove={event=>{const rect=event.currentTarget.getBoundingClientRect();event.currentTarget.style.setProperty('--room-x',`${((event.clientX-rect.left)/rect.width-.5)*2}`);event.currentTarget.style.setProperty('--room-y',`${((event.clientY-rect.top)/rect.height-.5)*2}`)}}>
          ...
          {activeSalonStation!==null&&<aside className="salon-insight" aria-live="polite"><button type="button" className="salon-insight-close" aria-label={lang==='de'?'Detail schließen':'Close detail'} onClick={()=>setActiveSalonStation(null)}>×</button><small>{c.salonInsightEye} · 0{activeSalonStation+1}</small><h3>{c.salonInsights[activeSalonStation][0]}</h3><p>{c.salonInsights[activeSalonStation][1]}</p><strong>{c.salonInsights[activeSalonStation][2]}</strong></aside>}
