    ...
    <main className={`experience-shell chapter-${chapter} ${entered?'has-entered':''} ${released ? 'is-released' : ''} ${reducedMotion?'reduce-motion':''}`}>
      ...
      <section className="atelier-scene scene" id="anwenden" aria-labelledby="atelier-title">
        ...
        <form className="controls" onSubmit={(event)=>event.preventDefault()}>
          ...
          <StudyIdentity intention={intention} setIntention={setIntention} statement={statement} setStatement={setStatement} labels={x}/>
...
function drawStudyArtwork(g:CanvasRenderingContext2D,box:{x:number;y:number;w:number;h:number},shape:{width:number;weight:number;layers:number},intention:number,statement:string){g.save();g.translate(box.x,box.y);g.fillStyle='#201512';g.fillRect(0,0,box.w,box.h);const seed=[...statement].reduce((sum,ch)=>sum+ch.charCodeAt(0),17)+shape.width*3+shape.layers*29;const dressW=box.w*(.28+(shape.width-32)/140);const cx=box.w/2;g.fillStyle='#e8dfcf';g.beginPath();g.arc(cx,box.h*.13,box.h*.065,0,Math.PI*2);g.fill();g.beginPath();g.moveTo(cx,box.h*.19);g.lineTo(cx-dressW/2,box.h*.86);g.quadraticCurveTo(cx,box.h*.96,cx+dressW/2,box.h*.86);g.closePath();g.fill();const colors=['#a54e31','#c49858','#2f6653','#17100e'];for(let i=0;i<8+shape.layers*3;i++){const px=cx-dressW*.42+((seed*(i+3)*17)%100)/100*dressW*.84;const py=box.h*.3+((seed*(i+7)*31)%100)/100*box.h*.5;const size=12+((seed+i*19)%34);g.fillStyle=colors[(i+intention)%colors.length];if((i+intention)%2){g.fillRect(px-size/2,py-size/2,size,size)}else{g.beginPath();g.arc(px,py,size/2,0,Math.PI*2);g.fill()}}g.strokeStyle=intention===0?'#a9d7c0':intention===1?'#c49858':'#e8dfcf';g.lineWidth=3+shape.layers;g.beginPath();for(let i=0;i<25;i++){const x=i/24*box.w;const y=box.h*(.18+intention*.12)+Math.sin(i*.7+seed)*box.h*(.035+shape.weight/1400);i?g.lineTo(x,y):g.moveTo(x,y)}g.stroke();g.restore()}

function StudyIdentity({intention,setIntention,statement,setStatement,labels}:{intention:number;setIntention:(v:number)=>void;statement:string;setStatement:(v:string)=>void;labels:typeof extraCopy.de|typeof extraCopy.en|typeof extraCopy.fr}){return <fieldset className="signature-maker identity-maker"><legend>{labels.signatureEye}</legend><h3>{labels.signatureTitle}</h3><p>{labels.signatureBody}</p><div className="intention-picker">{labels.intentions.map((item,i)=><button key={item} type="button" aria-pressed={intention===i} onClick={()=>setIntention(i)}>{item}</button>)}</div><label><span>{labels.statement}</span><input type="text" maxLength={70} value={statement} placeholder={labels.statement} onChange={e=>setStatement(e.target.value)}/></label><button className="identity-reset" type="button" onClick={()=>{setIntention(0);setStatement('')}}>{labels.signatureClear}</button></fieldset>}

function StudyPreview({shape,intention,statement}:{shape:{width:number;weight:number;layers:number};intention:number;statement:string}){const ref=useRef<HTMLCanvasElement>(null);useEffect(()=>{const canvas=ref.current;if(!canvas)return;const ratio=window.devicePixelRatio||1;const r=canvas.getBoundingClientRect();canvas.width=r.width*ratio;canvas.height=r.height*ratio;const g=canvas.getContext('2d');if(!g)return;g.scale(ratio,ratio);drawStudyArtwork(g,{x:0,y:0,w:r.width,h:r.height},shape,intention,statement)},[shape,intention,statement]);return <canvas className="study-preview" ref={ref} role="img" aria-label="Movement study preview"/>}
