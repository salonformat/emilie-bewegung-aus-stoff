...
const copy = {
  de: {
    ...
    businessEye:'Schwestern Flöge · Wien', businessTitle:['Gestalten heißt auch','entscheiden.'], businessLead:'Der Modesalon war zugleich Entwurfsraum, Geschäft und kultureller Treffpunkt. Die folgenden Perspektiven zeigen, welche Entscheidungen Modegestaltung als professionelle Praxis verlangt.', businessTasks:[['Silhouette','Form, Proportion und Bewegungsraum zu einer gestalterischen Haltung verbinden.'],['Material','Qualität, Wirkung und Herkunft von Textilien beurteilen und für eine Kollektion auswählen.'],['Salonpraxis','Aufträge, Beratung und Zusammenarbeit im laufenden Betrieb koordinieren.']], businessResult:['Die Silhouette übersetzt eine Haltung in Form – und bestimmt unmittelbar den Bewegungsraum.','Gewicht, Fall und Oberfläche geben dem Material seine Wirkung und bestimmen seinen Gebrauch.','Gestaltung verbindet Kundinnen, Mitarbeiterinnen und wirtschaftliche Entscheidungen im Alltag des Salons.'], businessLearning:'Flöge führte ihren Modesalon als eigenständige Gestalterin und Unternehmerin.', chooseTask:'Drei Perspektiven auf den Salon', reveal:'Perspektive', turnAgain:'Karte schließen', continueAtelier:'Zum digitalen Modeatelier',
    ...
    finalEye:'Schnitt, Stoff und Selbstständigkeit', finalTitle:'Kleidung bestimmt mit, wie frei sich ein Körper bewegen kann.', finalBody:'Emilie Flöges Arbeit verbindet Modegestaltung mit professioneller und unternehmerischer Eigenständigkeit.', quote:'„Schnitt, Material und Gewicht verändern nicht nur eine Silhouette – sondern auch ihren Bewegungsraum.“', again:'Experience erneut beginnen', prototype:'Prototyp · Arbeitsfassung 01', basis:'Historische Grundlage: Wien Museum und Belvedere · Illustration: künstlerische Interpretation'
  },
  en: {
...
const blankPatternCopy={de:'Wähle zuerst ein Motiv',en:'Choose a motif to begin',fr:'Choisissez une forme pour commencer'} as const;
...
function drawPatternedFigure(g:CanvasRenderingContext2D,img:HTMLImageElement,box:{x:number;y:number;w:number;h:number},motif:number,rhythm:number,accent:number){const off=document.createElement('canvas');off.width=Math.max(1,Math.round(box.w));off.height=Math.max(1,Math.round(box.h));const o=off.getContext('2d')!;const scale=Math.min(off.width/img.naturalWidth,off.height/img.naturalHeight);const dw=img.naturalWidth*scale,dh=img.naturalHeight*scale,dx=(off.width-dw)/2,dy=(off.height-dh)/2;o.drawImage(img,dx,dy,dw,dh);if(motif>=0){const pixels=o.getImageData(0,0,off.width,off.height),data=pixels.data,colors=[[165,78,49],[196,152,88],[47,102,83]],color=colors[accent],cell=Math.max(14,[62,45,32][rhythm]*off.width/500),mark=Math.max(4,cell*.42);for(let y=0;y<off.height;y++){for(let x=0;x<off.width;x++){const p=(y*off.width+x)*4,r=data[p],green=data[p+1],b=data[p+2],a=data[p+3];const neutralWhite=a>20&&r>224&&green>219&&b>202&&Math.abs(r-green)<25&&Math.abs(green-b)<32;if(!neutralWhite)continue;const fx=x%cell,fy=y%cell,square=fx<mark&&fy<mark,circle=Math.hypot(fx-cell/2,fy-cell/2)<mark*.72,stripe=Math.abs((fx+fy)%cell-cell/2)<Math.max(3,cell*.12),mix=(Math.floor(x/cell)+Math.floor(y/cell))%3,hit=motif===0?square:motif===1?circle:motif===2?stripe:mix===0?square:mix===1?circle:stripe;if(hit){data[p]=color[0];data[p+1]=color[1];data[p+2]=color[2]}}}o.putImageData(pixels,0,0)}g.drawImage(off,box.x,box.y,box.w,box.h)}

function StudyPreview({shape,intention,rhythm,accent,action='gehen',transparent=false}:{shape:{width:number;weight:number;layers:number};intention:number;rhythm:number;accent:number;action?:'gehen'|'arbeiten'|'tanzen';transparent?:boolean}){const ref=useRef<HTMLCanvasElement>(null);useEffect(()=>{let active=true;const canvas=ref.current;if(!canvas)return;const ratio=window.devicePixelRatio||1,r=canvas.getBoundingClientRect();canvas.width=Math.max(1,Math.round(r.width*ratio));canvas.height=Math.max(1,Math.round(r.height*ratio));const g=canvas.getContext('2d');if(!g)return;if(!transparent){g.fillStyle='#201512';g.fillRect(0,0,canvas.width,canvas.height)}loadPatternFigure().then(img=>{if(!active)return;g.clearRect(0,0,canvas.width,canvas.height);if(!transparent){g.fillStyle='#201512';g.fillRect(0,0,canvas.width,canvas.height)}drawPatternedFigure(g,img,{x:0,y:0,w:canvas.width,h:canvas.height},intention,rhythm,accent)});return()=>{active=false}},[shape,intention,rhythm,accent,action,transparent]);return <canvas className="study-preview" ref={ref} role="img" aria-label="Pattern and silhouette preview"/>}
