    ...
    <main className={`experience-shell chapter-${chapter} ${entered?'has-entered':''} ${released ? 'is-released' : ''} ${reducedMotion?'reduce-motion':''}`}>
      ...
      <section className="atelier-scene scene" id="anwenden" aria-labelledby="atelier-title">
        ...
        <div className="garment-lab">
          <div className={`lab-figure action-${action} ${mobility>=70?'is-achieved':''}`} style={{ '--dress-width':`${shape.width}%`, '--figure-scale':.92 + (shape.width-55)/360, '--weight-filter':1.08-shape.weight/360, '--motion-speed':`${1.35 + shape.weight/26}s`, '--motion-range':`${4 + mobility*.13}px`, '--motion-range-neg':`${-4-mobility*.13}px`, '--motion-lift-neg':`${-2-mobility*.09}px`, '--dance-angle':`${2+mobility*.025}deg`, '--dance-angle-neg':`${-2-mobility*.025}deg`, '--mobility':`${mobility}%`, '--layers':shape.layers } as React.CSSProperties}>
            <div className="motion-boundary" aria-hidden="true"/><div className={`lab-character motif-${intention} rhythm-${patternRhythm} accent-${patternAccent}`} onPointerMove={event=>{const rect=event.currentTarget.getBoundingClientRect();touchFabric((event.clientX-rect.left)/rect.width,.65)}} onWheel={event=>touchFabric((event.clientX-event.currentTarget.getBoundingClientRect().left)/event.currentTarget.getBoundingClientRect().width,.9)}><div className="silhouette-envelope" aria-hidden="true">{Array.from({length:shape.layers},(_,i)=><i style={{'--layer':i} as React.CSSProperties} key={i}/>)}</div><img className="lab-character-main" src="./visuals/emilie-figure-abstract.png" alt={c.portraitAlt}/><div className="dress-pattern" aria-hidden="true"/><span className="pattern-live-label">{x.preview}</span><img className="figure-motion figure-motion-upper" src="./visuals/emilie-figure-abstract.png" alt="" aria-hidden="true"/><img className="figure-motion figure-motion-skirt" src="./visuals/emilie-figure-abstract.png" alt="" aria-hidden="true"/></div><i className="floor-line" />
        ...
        <form className="controls" onSubmit={(event)=>event.preventDefault()}>
          ...
          <StudyIdentity intention={intention} setIntention={setIntention} rhythm={patternRhythm} setRhythm={setPatternRhythm} accent={patternAccent} setAccent={setPatternAccent} shape={shape} labels={x}/>
      ...
      <section className="final-scene scene" id="perspektive" aria-labelledby="final-title">
        ...
        <div className="final-copy">
          ...
          <div className="personal-study"><p className="eyebrow">{x.resultEye}</p><h3>{x.resultTitle}</h3><StudyPreview shape={shape} intention={intention} rhythm={patternRhythm} accent={patternAccent}/><p><strong>{c.actions[['gehen','arbeiten','tanzen'].indexOf(action)]}</strong> · {mobility}% {c.mobility}</p><p><strong>{x.intentions[intention]}</strong> · {x.rhythms[patternRhythm]} · {x.accents[patternAccent]}</p><p>{x.resultThought}</p><div><button type="button" onClick={downloadStudy}>{x.download}</button><button type="button" onClick={shareStudy}>{x.share}</button></div><small aria-live="polite">{shareStatus}</small></div>
...
function drawStudyArtwork(g:CanvasRenderingContext2D,box:{x:number;y:number;w:number;h:number},shape:{width:number;weight:number;layers:number},motif:number,rhythm:number,accent:number){g.save();g.translate(box.x,box.y);g.fillStyle='#201512';g.fillRect(0,0,box.w,box.h);const dressW=box.w*(.28+(shape.width-32)/140);const cx=box.w/2;g.fillStyle='#e8dfcf';g.beginPath();g.arc(cx,box.h*.13,box.h*.065,0,Math.PI*2);g.fill();g.beginPath();g.moveTo(cx,box.h*.19);g.lineTo(cx-dressW/2,box.h*.86);g.quadraticCurveTo(cx,box.h*.96,cx+dressW/2,box.h*.86);g.closePath();g.fill();g.save();g.clip();const colors=['#a54e31','#c49858','#2f6653'];g.fillStyle=colors[accent];g.strokeStyle=colors[accent];const step=[78,58,40][rhythm];for(let y=box.h*.23,row=0;y<box.h*.9;y+=step,row++){for(let x=cx-dressW/2+(rhythm===1&&row%2?step/2:0);x<cx+dressW/2;x+=step){const size=12+shape.layers*2;if(motif===0)g.fillRect(x-size/2,y-size/2,size,size);else if(motif===1){g.beginPath();g.arc(x,y,size/2,0,Math.PI*2);g.fill()}else{g.lineWidth=5;g.beginPath();g.moveTo(x-size,y+size);g.lineTo(x+size,y-size);g.stroke()}}}g.restore();g.strokeStyle='#17100e';g.lineWidth=3;g.stroke();g.restore()}

function StudyIdentity({intention,setIntention,rhythm,setRhythm,accent,setAccent,shape,labels}:{intention:number;setIntention:(v:number)=>void;rhythm:number;setRhythm:(v:number)=>void;accent:number;setAccent:(v:number)=>void;shape:{width:number;weight:number;layers:number};labels:typeof extraCopy.de|typeof extraCopy.en|typeof extraCopy.fr}){return <fieldset className="signature-maker identity-maker"><legend>{labels.signatureEye}</legend><h3>{labels.signatureTitle}</h3><p>{labels.signatureBody}</p><div className="pattern-workbench"><StudyPreview shape={shape} intention={intention} rhythm={rhythm} accent={accent}/><div className="pattern-choices"><span>1 · Motiv</span><div className="intention-picker">{labels.intentions.map((item,i)=><button key={item} type="button" aria-pressed={intention===i} onClick={()=>setIntention(i)}><i className={`motif-icon motif-icon-${i}`} aria-hidden="true"/>{item}</button>)}</div><span>2 · {labels.statement}</span><div className="intention-picker">{labels.rhythms.map((item,i)=><button key={item} type="button" aria-pressed={rhythm===i} onClick={()=>setRhythm(i)}>{item}</button>)}</div><span>3 · {labels.accent}</span><div className="intention-picker accent-picker">{labels.accents.map((item,i)=><button key={item} type="button" aria-pressed={accent===i} onClick={()=>setAccent(i)}><i className={`accent-dot accent-dot-${i}`} aria-hidden="true"/>{item}</button>)}</div></div></div><p className="pattern-connection">{labels.preview}: <strong>{labels.intentions[intention]} · {labels.rhythms[rhythm]} · {labels.accents[accent]}</strong></p><div className="pattern-actions"><button className="pattern-apply" type="button" onClick={()=>document.querySelector('#perspektive')?.scrollIntoView({behavior:'smooth'})}>{labels.signatureGenerate} →</button><button className="identity-reset" type="button" onClick={()=>{setIntention(0);setRhythm(0);setAccent(0)}}>{labels.signatureClear}</button></div></fieldset>}

function StudyPreview({shape,intention,rhythm,accent}:{shape:{width:number;weight:number;layers:number};intention:number;rhythm:number;accent:number}){const ref=useRef<HTMLCanvasElement>(null);useEffect(()=>{const canvas=ref.current;if(!canvas)return;const ratio=window.devicePixelRatio||1;const r=canvas.getBoundingClientRect();canvas.width=r.width*ratio;canvas.height=r.height*ratio;const g=canvas.getContext('2d');if(!g)return;g.scale(ratio,ratio);drawStudyArtwork(g,{x:0,y:0,w:r.width,h:r.height},shape,intention,rhythm,accent)},[shape,intention,rhythm,accent]);return <canvas className="study-preview" ref={ref} role="img" aria-label="Pattern and silhouette preview"/>}
